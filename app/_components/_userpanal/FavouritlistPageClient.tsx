...
export default function FavouritlistPageClient() {
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated);
  const list = useListStore((s) => s.getSystemList("favorites"));
  ...
  if (!isAuthenticated) {
    return (
      <SignInPrompt
        icon={<LuHeart className="size-12" />}
        description="Sign in to save your favorites and access them from any device."
      />
    );
  }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            ...
            <p className="text-second_text text-sm mt-1">
              {list.itemCount} {list.itemCount === 1 ? "favorite" : "favorites"}
            </p>
          </div>
        </div>
      </FadeIn>
