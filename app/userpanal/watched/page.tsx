...
function SignInPrompt() {
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
      <LuEye className="size-12 text-second_text/40 mb-4" />
      <h2 className="text-xl font-bold text-white mb-2">Sign in required</h2>
      <p className="text-second_text text-sm max-w-md mb-6">
        Sign in to start tracking what you&apos;ve watched.
      </p>
      <Link
        href="/signin"
...
export default function WatchedPage() {
  ...
  const list = useListStore((s) => s.getSystemList("watched"));
  ...
  const handleRemove = useCallback(
    async (item: ListItemWithMeta) => {
      ...
      const confirmed = window.confirm(
        `Remove "${item.title}" from your Watched list?`,
      );
      if (!confirmed) return;
      ...
      if (success) {
        toast.success(`Removed "${item.title}" from watched`);
      } else {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            ...
            <p className="text-second_text text-sm mt-1">
              {list.itemCount} {list.itemCount === 1 ? "title" : "titles"} watched
            </p>
          </div>
        </div>
      </FadeIn>
